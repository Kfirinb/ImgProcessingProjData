...
class Encoder(nn.Module):
    def __init__(self, latent_dims):
        ...
        #self.linear1 = nn.Linear(336528, 500)
        self.linear1 = nn.Linear(40000, 500)
        self.linear2 = nn.Linear(500, 350)
        self.linear3 = nn.Linear(350, latent_dims)
...
class Decoder(nn.Module):
    def __init__(self, latent_dims):
        ...
        self.linear2 = nn.Linear(512, 1125)
        #self.linear3 = nn.Linear(1125, 336528)
        ...
        for x, y in data:
            ...
            if epochs-1 == epoch:
                if imgs_counter < 16:
                    ...
                    x_hat.reshape((-1, 1, 200, 200))
                    #x_hattt = x_hat
                    x.reshape((-1, 1, 200, 200))
...
class binaryClassification(nn.Module):
    def __init__(self):
        ...
        # Number of input features is 200.
...
def main():
    ...
    transform = transforms.Compose([transforms.CenterCrop((200, 200)), transforms.Grayscale(1), transforms.ToTensor()])
    ...
    traindata, valdata, = random_split(dataset, [floor(0.01 * len(dataset)), ceil(0.99 * len(dataset))])  # checking
    ...
    fig.suptitle('Original img VS Cleaned by Decoder img')

    for i in range(len(comp_imgs_lst)):

        img = comp_imgs_lst[0][0,:,:].numpy()
        #img = comp_imgs_lst[0].numpy()
        comp_imgs_lst.pop(0)
    ...
    for i in range(len(comp_imgs_lst)):
        plt.subplot(5, 5, i + 1)  # the number of images in the grid is 50*50 (250)
        img_2d = comp_imgs_lst[0][0,:,:]
        #plt.imshow(img_2d.numpy()[0])
        plt.imshow(img_2d.permute(1, 2, 0))
        comp_imgs_lst.pop(0)
    """
    ...
    # set batch size to the data. shuffle isnt needed at that stage, as it was shuffled already
    ...
    # train data:
    model.train()
    for e in range(1, 50):  # epochs_num = 50
        epoch_loss = 0
        epoch_acc = 0
